...
class SensorBridgeCmdGetVersion(SensorBridgeCmdGetVersionBase):
    ...
    def interpret_response(data):
        ...
        :rtype: tuple
        """
        fw_ver_major = int(unpack(">1B", data[0:1])[0])  # uint8
        fw_ver_minor = int(unpack(">1B", data[1:2])[0])  # uint8
        fw_debug = bool(unpack(">1?", data[2:3])[0])  # bool
        hw_ver_major = int(unpack(">1B", data[3:4])[0])  # uint8
        hw_ver_minor = int(unpack(">1B", data[4:5])[0])  # uint8
        protocol_ver_major = int(unpack(">1B", data[5:6])[0])  # uint8
        protocol_ver_minor = int(unpack(">1B", data[6:7])[0])  # uint8
        return fw_ver_major,\
            fw_ver_minor,\
            fw_debug,\
            hw_ver_major,\
